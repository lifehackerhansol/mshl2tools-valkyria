...
u16 g_ptr;
TPluginBody *g_PB;
mm_word on_stream_request_msp( mm_word length, mm_addr dest, mm_stream_formats format ) {
	s16 *target = dest;
...
void swapcartbeforeexec(u8* buf){
	...
	for(swiWaitForVBlank();;swiWaitForVBlank()){
		scanKeys();
		if(!(keysDown()))continue;
		if(keysDown()&KEY_A)break;
		if(keysDown()&KEY_B)systemShutDown();
	}
...
void notice(){
	...
	_consolePrint2(
		...
		"under 2-clause BSDL.\n"
#endif
		"Powered by libvalkyria "ROMVERSION"\n"
		ROMDATE"\n"
		ROMENV"\n\n"
		//"[loader]\n"
		"reset_mse_06b_for_ak2 by Moonlight, Rudolph, kzat3\n"
...
//#endif
	...
	for(;;){
		...
		for(;;swiWaitForVBlank()){
			scanKeys();
			if(!keysHeld())continue;
			if(key=keysDown()){keycountmax=KEYCOUNTMAX_UNREPEATED;break;}
			keycount++;
			if(keycount==keycountmax){key=keysHeld()&0xf0;keycountmax=KEYCOUNTMAX_REPEATED;break;}
		}
		keycount=0;
...
bmp_fail:
			...
			if(strlen(ext[i])&&strlen(p->name)>=strlen(ext[i])&&!strcasecmp(p->name+strlen(p->name)-strlen(ext[i]),ext[i])){
				_consolePrint2("Sorry but u8m support was removed in FIFO version.\n");
#if 0
				struct stat st;
				FILE *f;
				strcpy(file,dir);
...
//_consolePrintf2("%08x\n",sampleperframe);
					...
					while(scanKeys(),!keysDown()){ //mainloop.
						// wait until line 0
...
msp_sound_end:
				if(PB){
					mmStreamClose();
					DisableB15Main();
					g_PB=NULL;DLLList_FreePlugin(PB);
				}
				fclose(f);
				getfilelist(dir,filter);
				usage();
			...
			switch(ret){
				...
				case op_delete:{
					...
					if(st.st_mode&S_IFDIR){
						...
						for(;scanKeys(),!(key=keysDown());)swiWaitForVBlank();
						if(!(key&KEY_A)){_consolePrint2("Aborted.\n");break;}
						for(swiWaitForVBlank();scanKeys(),!(key=keysDown());)swiWaitForVBlank();
						if(!(key&KEY_A)){_consolePrint2("Aborted.\n");break;}
						strcat(file,"/");
						rm_rf(file);
						...
						int key;
						for(;!(key=keysDown());)swiWaitForVBlank();
						if(!(key&KEY_A)){_consolePrint2("Aborted.\n");break;}
					}
					unlink(file);
					while(scanKeys(),keysHeld())swiWaitForVBlank();
...
rename_retry:
					...
					touchPosition touch;
					for(swiWaitForVBlank();;swiWaitForVBlank()){
						scanKeys();touchRead(&touch);
						if(keysDown()&KEY_A){_consolePrintChar2('\n');break;}
						if(keysDown()&KEY_B){_consolePrintChar2('\n');goto rename_cancel;}
						if(keysDown()&KEY_TOUCH &&
							0<=touch.px&&touch.px<=255 &&
							96<=touch.py&&touch.py<=191
						){
							u8 key=((caps|shift)?keyboard_Hit_Shift:keyboard_Hit)[((touch.py-96)/8)*32+touch.px/8];
							if(!key)continue;
							if(key==RET){
								_consolePrintChar2('\n');break;
							}else if(key==SHF){
...
attr_again:
					...
					for(swiWaitForVBlank();;swiWaitForVBlank()){
						scanKeys();
						if(keysDown()&KEY_A){_consolePrint2("Yes.\n");newattr|=ATTRIB_RO;break;}
						if(keysDown()&KEY_B){_consolePrint2("No.\n");break;}
					}
					...
					for(swiWaitForVBlank();;swiWaitForVBlank()){
						scanKeys();
						if(keysDown()&KEY_START){goto attr_apply;}
						if(keysDown()&KEY_SELECT){goto attr_cancel;}
						if(keysDown()&KEY_B){goto attr_again;}
					}
		...
		if(key&KEY_Y){
			...
			switch(ret){
				...
				case sys_systeminfo:{
					...
					_consoleClear2();
					u8 batt=getBatteryLevel();
					time_t ttt=time(NULL);
					strftime(stime, 30, "%Y-%m-%d (%a) %H:%M:%S", localtime(&ttt));
					*fwver=0;
					if(fwinfo.flashmeversion==1)strcpy(fwver,"FlashMe old");
					if(fwinfo.flashmeversion>1)sprintf(fwver,"FlashMe v%d",fwinfo.flashmeversion+3);
					if(!fwinfo.fwchksum)strcpy(fwver,"DS(Lite) Preferences");
					if(!*fwver){
						int iVer=GetFirmwareVersion();
						if(!iVer)sprintf(fwver,"Unknown (%04x)",fwinfo.fwchksum);
						else sprintf(fwver,
							iVer==FW_iQue?"iQue":
							iVer==FW_Korean?"Korean":
							"v%d",iVer);
					}
					u32 u_temper=CallARM7(1,RequestTemperature);
					sprintf(temper,u_temper/0x1000>1000?"Not Available":"%.2f C",(double)u_temper/0x1000);
					...
					_consolePrintf2(
						...
						"AutoMode:    %d\n",
						//"rtcOffset:   0x%08x\n",
						fwinfo.NDSType==NDSPhat?"DS (Phat)":fwinfo.NDSType==NDSLite?"DS Lite":
						fwinfo.NDSType==NDSi?(GetRunningMode()==1?"DSi (DSi mode)":"DSi (DS mode)"):
						fwinfo.NDSType==ThreeDS?(GetRunningMode()==2?"3DS (3DS mode)":GetRunningMode()==1?"3DS (DSi mode)":"3DS (DS mode)"):
						"New Device",
						fwver,fwinfo.fwsize,
						ram_type_string(),ram_size()>>10,
						batt&0xf,batt&BIT7?", charging":"",
						fwinfo.MAC[0],fwinfo.MAC[1],fwinfo.MAC[2],fwinfo.MAC[3],fwinfo.MAC[4],fwinfo.MAC[5],
						//fwinfo.dslite,
						stime,
						temper,
...
char *region[]={
	...
	"EUU:EuropeUnion", //Z
				...
				case sys_changedldi:{
					...
					DC_FlushAll();
					DC_InvalidateAll();
					_consolePrint("Reinitializing libfat...\n");
				...
				case sys_bios:{
					...
					u8 *p=(u8*)malloc(fwinfo.fwsize);
					if(!p){_consolePrint2("Cannot alloc buffer.\n");goto bios_end;}
					DC_FlushAll();
					CallARM7(3,GetFirmware,p,fwinfo.fwsize);
					DC_InvalidateAll();

					f=fopen(fwname,"wb");
					int i=0;
					_consoleStartProgress2();
					for(;i<fwinfo.fwsize;i+=min(BUFLEN,fwinfo.fwsize-i),_consolePrintProgress2("Writing",i,fwinfo.fwsize)){
						fwrite(p+i,1,min(BUFLEN,fwinfo.fwsize-i),f);
					}
					_consoleEndProgress2();
					free(p);
					fclose(f);
...
bios_end:
				...
				case sys_testsd:{
					...
					timerStart(2, ClockDivider_64, TIMER_FREQ_64(1000), Timer2Handler);
					//OK test from here
