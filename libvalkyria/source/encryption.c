...
NOINLINE void EnDecryptSecureArea(const char *ndsfilename, char endecrypt_option){
	...
	if (!fNDS) { PRINTF("Cannot open file '%s'.\n", ndsfilename); return; }
...
*/
			...
			for(i=0x3000; i<0x3008; i++) fputc("\xFF\x00\xFF\x00\xAA\x55\xAA\x55"[i - 0x3000], fNDS);
			for(i=0x3008; i<0x3200; i++) fputc((u8)i, fNDS);
			for(i=0x3200; i<0x3400; i++) fputc((u8)(0xFF-i), fNDS);
			for(i=0x3400; i<0x3600; i++) fputc(0x00, fNDS);
			for(i=0x3600; i<0x3800; i++) fputc(0xFF, fNDS);
			for(i=0x3800; i<0x3A00; i++) fputc(0x0F, fNDS);
			for(i=0x3A00; i<0x3C00; i++) fputc(0xF0, fNDS);
			for(i=0x3C00; i<0x3E00; i++) fputc(0x55, fNDS);
			for(i=0x3E00; i<0x4000-1; i++) fputc(0xAA, fNDS);
			fputc(0x00, fNDS);
	...
	else if (romType >= ROMTYPE_ENCRSECURE)		// includes ROMTYPE_MASKROM
	{
		...
		else
		{
			...
			//DSi safe
			fseek(fNDS, 0x1000, SEEK_SET);
			for (i=0x1000; i<0x4000; i++) fputc(0, fNDS);
...
void EncryptSecureArea(const char *ndsfilename){EnDecryptSecureArea(ndsfilename, 'e');}
void DecryptSecureArea(const char *ndsfilename){EnDecryptSecureArea(ndsfilename, 'd');}
