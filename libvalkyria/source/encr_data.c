static const unsigned char __device_tab[8] = { 0xE8, 0x4D, 0x5A, 0xB1, 0x17, 0x8F, 0x99, 0xD5 };
static const unsigned char __encr_data[0x1048] = { //from ndstool. should be copyrighted material...
...
#if !defined(ARM9)
const unsigned char *encr_data=__encr_data;
#else
#include "../libvalkyria.h"
const unsigned char key_tbl[0x4000];
const unsigned char *encr_data=key_tbl+0x30; //latter 0x1048bytes are used.
static int inited=0;
void InitializeKeyTable(){
	...
	if(GetRunningMode()){
		...
		CallARM7(3,GetARM7Bios,key_tbl,0x4000); //good bye. with iEvo's DSi->DS compat layer, freezes here!
	}
}
#endif
