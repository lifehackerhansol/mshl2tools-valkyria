...
void bootMoonlight(u32 BootAddress){
	...
	u32 arm7addr=read32((u8*)BootAddress-0x8c);
	u32 arm7end=arm7addr+read32((u8*)BootAddress-0x84);

  	//_consolePrintf("%08x\n",arm7end);
	//for(scanKeys();!keysDown();scanKeys())swiWaitForVBlank();
	...
	if(
		arm7addr>=0x03800000 ||
		(arm7addr==0x037f8000 && 0x03800000<arm7end && arm7end<=0x03808000) //PAlib app should exceed this
	){ //Possibly MoonShell2.
		IC_InvalidateAll();
		DC_FlushAll();
		DC_InvalidateAll();
		NotifyARM7(3,ResetMoonlight,BootAddress,1);
		//_consolePrint("Waiting ARM7...\n");
		//wait for resetARM9()
	}else{
