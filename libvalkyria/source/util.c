...
int main(int __argc, char **__argv){
	...
	REG_MASTER_BRIGHT_SUB = REG_MASTER_BRIGHT = 0; // I never know about side-effects.
#else
...
*/
	...
	_consolePrint2("Grabbing fwinfo... ");
	int size=align4(sizeof(fwinfo))/4;
	fifoSendValue32(FIFO_USER_03,size);
	int i=0;
	u32 *p=(u32*)&fwinfo;
	_consolePrintf2("size=%d. ",size);
	for(;i<size;)
		if(fifoCheckValue32(FIFO_USER_03)){
			p[i++]=fifoGetValue32(FIFO_USER_03);
			_consolePrintf2("%d ",i);
		}
...
retry:
	...
	for(swiWaitForVBlank();;swiWaitForVBlank()){
		scanKeys();
		if(KEY_A&keysHeld())disc_unmount(),systemShutDown();
		if(h&&(KEY_B&keysHeld()))
...
void NotifyARM7(u32 argc, ...){
	va_list list;
	u32 i=0;
	fifoSendValue32(FIFO_USER_01,argc);
	_consolePrintf("Sent: %08x\n",argc);
	va_start(list, argc);
	for(i=0;i<argc;i++){
		u32 arg=va_arg(list, u32);
		fifoSendValue32(FIFO_USER_01,arg);
		_consolePrintf("Sent: %08x\n",arg);
	}
	va_end(list);
	_consolePrintf("Waiting\n");
	while(!fifoCheckValue32(FIFO_USER_01));
	u32 x=fifoGetValue32(FIFO_USER_01);
	_consolePrintf("ARM7: %u\n",x);
}

u32 CallARM7(u32 argc, ...){
	va_list list;
	u32 i=0;
	fifoSendValue32(FIFO_USER_02,argc);
	va_start(list, argc);
	for(i=0;i<argc;i++){
		fifoSendValue32(FIFO_USER_02,va_arg(list, u32));
	}
	va_end(list);
	while(!fifoCheckValue32(FIFO_USER_02));
	return fifoGetValue32(FIFO_USER_02);
}
